            ...
            try {
                ...
                const imageRequests = data.map(breed =>
                    breed.reference_image_id
                        ? fetch(`https://api.thedogapi.com/v1/images/${breed.reference_image_id}`).then(res => res.json())
                        : Promise.resolve({ id: null, url: 'https://via.placeholder.com/150' })
                );
            ...
            {!dogs.length ? (
                ...
                <section className="p-8 max-w-7xl mx-auto">
                    <div className="text-center">
                        <h1 className="flex items-center justify-center text-white text-center px-5 text-2xl font-bold lg:text-2xl">
                            The Dog App
                        </h1>
                        <p className="my-8 text-white">
                            This application is powered by{' '}
                            <a href="https://thedogapi.com" className="text-indigo-100 underline active:text-orange-400">
                                The Dog API
                            </a>
                        </p>
                    ...
                    <div className='grid grid-cols-1 gap-8 md:grid-cols-2 xl:grid-cols-3 my-10 lg:my-20'>
                        {paginatedDogs.map(dog => (
                            <Link to={`/dog/${dog.name}`} key={dog.id} className="group relative overflow-hidden transition-shadow hover:shadow-xl hover:shadow-black/30">
                            
                                
                                <article className="mb-8">
                                    <img
                                        src={images[dog.reference_image_id] || 'https://via.placeholder.com/150'}
                                        alt={dog.name}
                                   ...
                                   <div className="absolute inset-0 flex flex-col items-center justify-center px-4 text-center transition-all duration-500 group-hover:translate-y-0 translate-y-[60%]">
                                    <h2 className="text-white text-lg font-semibold mt-8 ">{dog.name}</h2>
